SUBROUTINE pp_cnvln_tapered(taper_cut_off,taper_range,alpha_cnvltn,norm,nbf,np,na,ng,grdpts,g,bs,cs,po,eigen,ueff)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 DOUBLE PRECISION, INTENT(IN) :: eigen, alpha_cnvltn, taper_cut_off, taper_range
 DOUBLE PRECISION, INTENT(IN) :: bs(np,10), cs(na+ng,3), po(nbf), g(grdpts,3)
 DOUBLE PRECISION, INTENT(OUT) :: ueff(grdpts)
 DOUBLE PRECISION :: ueff_tapered
 ...
 DOUBLE PRECISION :: charges(na,1), molc(na,3)
 INTEGER :: prim, i, j, k, cntr, atom, xe, ye, ze

 CALL READ_FILE('point_charges.in',na,1,charges)
 
 OPEN(UNIT=134,FILE="pk_potential_smoothed_tapered.out",STATUS="REPLACE",ACTION="WRITE")
! 100 FORMAT(E14.6,2X,E14.6,2X,E14.6,2X,E14.6E3)
 100 FORMAT(E14.6E3)

 DO i=1,na
  molc(i,1) = cs(i,1); molc(i,2) = cs(i,2); molc(i,3) = cs(i,3)
 END DO
 ...
 DO k=1,grdpts
  ...
  ueff(k) = eigen - kin_orb/orb
  CALL TAPER_ATOM(na,taper_cut_off,taper_range,charges,x,y,z,molc,ueff(k),ueff_tapered)
!  WRITE(134,100) x, y, z, ueff_tapered
  WRITE(134,100) ueff_tapered
 END DO
 CLOSE(134)
